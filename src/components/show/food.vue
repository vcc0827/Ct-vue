  ...
  <div class="container">
    ...
    <el-carousel :interval="4000" type="card" height="300px">
      <el-carousel-item v-for="item in dat" :key="item">
        <img :src="`http://localhost:3000/hkfood/`+item" alt="" class="image">
      </el-carousel-item>
    </el-carousel>
    <h3 style=" text-align: center">美食频道-----香港专区</h3>
    <el-row style="width: 100%;margin: 0 auto;">
      <el-col :span="6" class="ul" v-for="(item ,key,index) in data" v-if="item.id<=4" :key="key" :index="index">
        <div class="li">
          <img :src="`http://localhost:3000/hkfood/`+item.src" alt="" class="img">
    ...
    <el-row style="width: 100%;margin: 0 auto">
      <h3 style="width: 100%;text-align: center">美食频道-----巴黎专区</h3>
      <el-col :span="6" class="ul" v-for="(item,key,index) in data" v-if="item.id>4" :key="key" :index="index">
        <div class="li">
          <img :src="`http://localhost:3000/hkfood/`+item.src" alt=""  class="img">
          <span style="font-weight: bold;font-size: 16px">{{item.name}}</span>
